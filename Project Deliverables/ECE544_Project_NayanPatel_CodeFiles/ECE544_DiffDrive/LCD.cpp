...
void lcdInit(void){
  ...
  // lcd.clear();  
  lcd.setCursor (0, 0); //
  lcd.print("This is a test run"); 
  lcd.setCursor (0, 1); //
  lcd.print("for Differential"); 
  lcd.setCursor (0, 2); //
  lcd.print("Drive Robot"); 
...
void printLCDScreen(lcdData data){
  ...
  dtostrf(data.robotTargetPos_X, 3, 1, X);
  lcd.print(X); // print text 
  lcd.setCursor (8, 1); //character zero, line 0
  lcd.print("|Xa:"); // print text  
  lcd.setCursor (13, 1); //character 14, line 0
  dtostrf(data.robotPos_X, 3, 1, X);
  lcd.print(X); // print text 
  delay(0.01);  
  ...
  dtostrf(data.robotTargetPos_Y, 3, 1, Y);
  lcd.print(Y); // print text
  lcd.setCursor (8, 2); //character zero, line 1  
  lcd.print("|Ya:"); // print text  
  lcd.setCursor (13, 2); //character 14, line 1
  dtostrf(data.robotPos_Y, 3, 1, Y);
  lcd.print(Y); // print text
  delay(0.01);     
  ...
  dtostrf(data.imuTargetOrientiation_X, 4, 1, Theta1);
  lcd.print(Theta1); // print text
  lcd.setCursor (8, 3); //character zero, line 2  
  lcd.print("|Ta:"); // print text  
  lcd.setCursor (13, 3); //character 4, line 0
  dtostrf(data.imuOrientiation_X, 4, 1, Theta1);
  lcd.print(Theta1); // print text
  delay(0.01); 
}